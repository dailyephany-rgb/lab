...
// ------------------------------------------------------
// Hormones (Main) — STRICT Time Printed Implementation
// ------------------------------------------------------
...
export function isHormonesMainTest(testName) {
  ...
  return HORMONES_FULL_NAMES.some((canonical) => {
...
export function computeSLAViolations(unifiedRows, timingMap, stage = "scanned_to_saved") {
  ...
  unifiedRows.forEach((row) => {
   const allowed = timingMap["hormones"]?.[stage] ?? 
  timingMap.default?.[stage] ?? 30;
