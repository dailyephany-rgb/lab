...
import {
  ...
  toDate,
  minutesDiff,           // Added import to calculate delay
  fetchTestTimings,       
...
export default function OwnerBiochem() {
  ...
  const [fetchedKpis, setFetchedKpis] = useState(null); // Renamed for clarity
  ...
  const slowestEntry = useMemo(() => {
    let slowest = null;
    deptRows.forEach((r) => {
      // Calculate delay from Scanned to Saved
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...fetchedKpis,
      slowestEntry: slowestEntry // Inject the locally calculated slowest entry
    };
