...
// Updated to use the cloned Urine Data Fetcher
import {
  subscribeOverview,
  fetchTestTimings,
  toDate,
...
export default function OwnerUrinePage() {
  ...
  const [violators, setViolators] = useState([]); // Captures strict > 30 mins list from fetcher
  ...
  useEffect(() => {
    const unsub = subscribeOverview({
      ...
      onData: (payload = {}) => {
        ...
        // FIX: Capture pre-filtered violators from fetcher (Strict check)
  ...
  const kpisToUse = useMemo(() => {
    ...
    // FIND SLOWEST ENTRY FROM VIOLATORS
  ...
  return (
    <div className="owner-root">
      <header className="owner-header">
        ...
        {/* 🛠️ UPDATED: Added tab-buttons wrapper for the new look */}
